...
export default function Chat() {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      if (initialPrompt && !hasProcessedInitialPromptRef.current) {
        ...
        setTimeout(() => {
          console.log('Sending initial prompt to agent')
          handleSendMessageWithText(initialPrompt)
          // Clear the URL parameter to prevent double submission
          router.replace('/chat')
        }, 500)
      }
    }
  ...
  const handleSendMessageWithText = async (text: string) => {
    ...
    // Check if this exact message was already sent recently
    const recentMessages = messages.slice(-3) // Check last 3 messages
    const isDuplicate = recentMessages.some(msg => 
      msg.sender === 'user' && msg.text === text
    )
    ...
    try {
      ...
      const ssePromise = new Promise<void>((resolve, reject) => {
        const directUrl = `http://localhost:8000/api/agent/chat/stream-sse?message=${encodeURIComponent(text)}`
  ...
  const handleSendMessage = async () => {
    ...
    await handleSendMessageWithText(inputText)
    setInputText('')
  }
  ...
  const handleClearSession = async () => {
    try {
      // Call the reset session API endpoint
      await axios.post('/api/agent/reset-session')
      
      // Navigate back to the landing page
      router.push('/')
    } catch (error) {
      console.error('Failed to reset session:', error)
      // Still navigate back to landing page even if API call fails
      router.push('/')
    }
  }
        ...
        <header className="p-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs text-red-600 hover:text-red-800 px-2 py-1 rounded hover:bg-red-50 border border-red-200"
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((message) => (
              ...
              >
                {message.sender === 'user' ? (
                  <div className="chat-bubble user-bubble max-w-xs lg:max-w-md">
                    <div className="text-base">{formatMessage(message.text)}</div>
                  </div>
                ) : (
                  <div className="w-full">
                    <div className="text-base text-gray-800">{formatMessage(message.text)}</div>
                  </div>
                )}
              </div>
            ))}
